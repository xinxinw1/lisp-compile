...
(function (win, udf){
  ...
  function mblk(a, ret){
    if (nilp(a))return [];
    return mblk2(rem(nilp, a), ret);
  }
  
  function mblk2(a, ret){
    if (nilp(a))return [];
    if (atmp(a))err(mblk2, "Can't cmp improper list a = $1", a);
    if (nilp(cdr(a))){
    ...
    var c = cmp(car(a), ret);
    if (lisp(c))return app(c, mblk2(cdr(a), ret));
    return cons(c, mblk2(cdr(a), ret));
  }
  ...
  function chkrt(a, cr){
    if (blk){
      if (thp){
        if (nilp(a))return "throw;\n";
        return "throw " + a + ";\n";
      }
      if (rtp){
        if (nilp(a))return "return;\n";
        return "return " + a + ";\n";
      }
      if (nilp(a))return [];
      return blkbrc(a, cr) + ";\n";
  ...
  function clop(a){
    ...
    s += cmp(cadr(a), "bot") + "; ";
    s += cmp(nth("2", a), "bot") + ")";
    return lis(s + chkpar(mblk(ncdr("3", a), "lop")));
  }
  
  function cwhi(a){
    var s = "while (" + cmp(car(a), "bot") + ")";
    return lis(s + chkpar(mblk(cdr(a), "lop")));
  }
  ...
  function cfoi(a){
    ...
    s += " in ";
    s += cmp(cadr(a), "bot") + ")";
    return lis(s + chkpar(mblk(cddr(a), "lop")));
  }
